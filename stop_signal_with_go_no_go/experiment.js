...
var appendData = function(){
	...
	if (exp_phase == "test"){	
		...
		} else if ((jsPsych.data.getDataByTrialIndex(curr_trial).key_press != jsPsych.data.getDataByTrialIndex(curr_trial).correct_response) && (jsPsych.data.getDataByTrialIndex(curr_trial).stop_condition == 'go')){
...
var practiceNode = {
	...
	loop_function: function(data) {
		...
		var stop_length = 0
		for (i = 0; i < data.length; i++) {
			if (data[i].trial_id == "practice_no_stop_trial") {
				go_length += 1
				if (data[i].rt != -1) {
					num_responses += 1
					sum_rt += data[i].rt;
		...
		if ((averageGo_correct >= accuracy_thresh)||(practiceCount == 3)){
			...
			if (missed_responses > missed_response_thresh){
				...
				feedback_text +=
					'</p><p class = block-text>We have detected a number of trials that <i>required a response</i>, where no response was made.  Please <i>ensure that you are quickly responding </i>to the shapes.'
				} else {
				feedback_text +=
					'</p><p class = block-text>We have detected a number of trials that <i>required a response</i>, where no response was made.  Please <i>ensure that you are quickly responding </i>to the shapes.<br>' +
					prompt_text
				}
			}
			...
			if (average_rt > rt_thresh) {
				feedback_text +=
				'</p><p class = block-text>You have been responding too slowly, please respond to each shape as quickly and as accurately as possible.</i>'
			}
			if (missed_responses > missed_response_thresh){
				if(averageGo_correct < accuracy_thresh){
				feedback_text +=

...
function addID() {
  jsPsych.data.addDataToLastTrial({exp_id: 'stop_six', subject_ID: subject_ID})
}
...
var possible_responses = [['Q key', 81], ['W key', 87], ['E key', 69], ['I key', 73], ['O key', 79], ['P key', 80]]
var stop_six_response = ['No Response', -1]


var postFileType = "'></img>"
var pathSource = "/static/experiments/stop_six/images/"
var fileType = ".png"
...
var prompt_text = '<ul list-text>'+
					...
					'<li>' + shapes[4] + ': ' + possible_responses[4][0] + '</li>' +
					'<li>' + shapes[5] + ': ' + possible_responses[5][0] + '</li>' +
				  '</ul>'
...
var practice_stop_intro = {
	...
	stimulus: '<div class = centerbox>'+
			  	...
			  	'<p class = block-text>On some trials, a star will appear around the shape.  If the star appears, please make ' + stop_six_response[0] + '.</p>'+
				...
				prompt_text +
				'<p class = block-text>If you see a star, please make ' + stop_six_response[0] + ' on that trial.</p>'+
...
for (i = 0; i < practice_length; i++) {
	...
	var practiceBlock = {
		...
		choices: [possible_responses[0][1],possible_responses[1][1],possible_responses[2][1],possible_responses[3][1],possible_responses[4][1],possible_responses[5][1]],
		timing_stim: 850,
		timing_response: 850,
	...
	loop_function: function(data) {
		...
		for (i = 0; i < data.length; i++) {
			if (data[i].stop_type == "go") {
				...
				stop_length += 1
				if (data[i].key_press == stop_six_response[1]) {
					sumStop_correct += 1
				}
			}
		}
	...
	loop_function: function(data) {
		...
		if (testCount == numBlocks) {
			...
			if (stop_respond > 0.70){
				feedback_text +=
					'</p><p class = block-text>We have detected a number of trials that <strong>required ' + stop_six_response[0] + '</strong>, where a ' + possible_responses[0][0] + ' or a ' + possible_responses[1][0] + ' response was made.  Please <strong>ensure that you are making ' + stop_six_response[0] + ' </strong> when you see a star.'
...
var stop_six_experiment = []

stop_six_experiment.push(welcome_block);
stop_six_experiment.push(instructions_block);

stop_six_experiment.push(practice_intro);
stop_six_experiment.push(practiceNode);
stop_six_experiment.push(feedback_block);

stop_six_experiment.push(practice_stop_intro)
stop_six_experiment.push(practiceStopNode)
stop_six_experiment.push(feedback_block);

stop_six_experiment.push(test_intro);
stop_six_experiment.push(testNode);
stop_six_experiment.push(feedback_block);

stop_six_experiment.push(end_block);

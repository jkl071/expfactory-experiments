...
var getTestFeedback = function() {
	...
	test_feedback_text = "<br>Done with a test block. Please take this time to read your feedback and to take a short break! Press <i>enter</i> to continue after you have read the feedback."
	test_feedback_text += "</p><p class = block-text><i>Average reaction time:  " + Math.round(average_rt) + " ms. Accuracy for non-star trials: " + Math.round(GoCorrect_percent * 100)+ "%</i>" 
	...
	if (missed_responses >= missed_response_thresh) {
		test_feedback_text +=
			'</p><p class = block-text><i>We have detected a number of trials that required a response, where no response was made.  Please ensure that you are responding to each set of letters, unless a star appears.</i>'
	}
	...
	if (StopCorrect_percent < (0.5-stop_thresh) || stopAverage < 0.45){
			 	test_feedback_text +=
			 		'</p><p class = block-text><i>Remember to try and withhold your response when you see a stop signal.</i>'	
	} else if (StopCorrect_percent > (0.5+stop_thresh) || stopAverage > 0.55){
	 	test_feedback_text +=
	 		'</p><p class = block-text><i>Remember, do not slow your responses to the the letters to see if a star will appear before you respond.  Please respond to each letter set as quickly and as accurately as possible.</i>'
	}
...
var practice2 = {
	...
	stimulus: '<div class = bigbox>'+
				'<div class = instructBox>'+
					...
					'<p class = block-text style="font-size:22px; line-height:100%;">If a star appears, please try your best <i>not to respond</i> on that trial.</p>'+
...
var instructions_block = {
	...
	pages: [
		'<div class = centerbox>' + 
			...
			"<p class = block-text>Your task is to respond by pressing the key corresponding to the <i>middle</i> letter. </p>"+
...
var start_test_block = {
	...
	text: '<div class = centerbox>'+
			...
			'<p class = block-text>If you see a star appear, please try your best to <i>make no response</i> on that trial.</p>'+
...
var NoSS_practice_node = {
	...
	loop_function: function(data) {
		...
		var GoCorrect_percent = sum_correct / go_length;
		var missed_responses = (go_length - num_responses) / go_length
		practice_feedback_text = "</p><p class = block-text><i>Average reaction time:  " + Math.round(average_rt) + " ms. Accuracy for non-star trials: " + Math.round(GoCorrect_percent * 100)+ "%</i>" 
		if ((average_rt < RT_thresh && GoCorrect_percent > accuracy_thresh && missed_responses <
			...
			practice_feedback_text +=
				'</p><p class = block-text>For the rest of the experiment, on some proportion of trials a black "stop signal" in the shape of a star will appear around the string of letters. When this happens please try your best to stop your response and press nothing on that trial.</p><p class = block-text>The star will appear around the same time or shortly after the letters appear. Because of this, you will not always be able to successfully stop when a star appears. However, if you continue to try very hard to stop when a star appears, you will be able to stop sometimes but not always.</p><p class = block-text><i>Please balance the requirement to respond quickly and accurately to the letters while trying very hard to stop to the stop signal.</i></p><p class = block-text>Press <i>Enter</i> to continue'
			return false;
		} else {
			...
			if (missed_responses >= missed_response_thresh) {
				practice_feedback_text +=
					'</p><p class = block-text><i>We have detected a number of trials that required a response, where no response was made.  Please ensure that you are responding to each string.</i>'
			}
			if (GoCorrect_percent <= accuracy_thresh) {
				practice_feedback_text +=
					'</p><p class = block-text>Your accuracy is too low.'
			}
			practice_feedback_text += '</p><p class = block-text>Press <i>Enter</i> to continue'
			return true;
		}
	}
}
...
practice_trials.push(instructions_block)
	...
	loop_function: function(data) {
		...
		if ((average_rt < RT_thresh && GoCorrect_percent > accuracy_thresh && missed_responses <
			...
			practice_feedback_text +=
				...
				numblocks +
				' test blocks. There will be a break after each block. Press <i>enter</i> to continue.'
			return false;
		} else {
			...
			if (missed_responses >= missed_response_thresh) {
				practice_feedback_text +=
					'</p><p class = block-text><i>We have detected a number of trials that required a response, where no response was made.  Please ensure that you are responding to each string of letters, unless a star appears.</i>'
			}
			...
			if (StopCorrect_percent < 0.8){
			 	...
			 	practice_feedback_text +=
			 		'</p><p class = block-text><i>Remember, do not slow your responses to the letters to see if a star will appear before you respond.  Please respond to each string as quickly and as accurately as possible.</i>'
			}
			practice_feedback_text += '</p><p class = block-text>Press <i>Enter</i> to continue'
			return true;
		}

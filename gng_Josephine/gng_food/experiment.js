...
//makes that button press equivalent to the a key press
...
//functions that get the finalSample(stims that will be sorted to conditions)
...
//Sorts the stims based on their rating
...
//Grab stims to sort to groups (pop gets the high rated stim and shift gets the low)
...
//Uses the above functions to split stims into 6 groups of equal size
...
//Randomly assign groups to conditions
...
//creates an array containing 5 arrays that each hold all the stims of the given condition
...
function getTaskGoStims5 () { 
	stim = goSamples[4].pop()
	return preTrialPic+stim+postTrialPic
}; 

function getTaskNoGoStims5 () {  
	stim=noGoSamples[4].pop() 
	return preTrialPic+stim+postTrialPic
}; 

//pulls together the Stim Sample for the Post Task Evaluation
...
var test_stimuli_block5 = [{
  ...
  prompt: getTaskNoGoStims5
}];

for (var i = 0; i < num_go_stim; i++) {
  test_stimuli_block5.push({
    stimulus: pathSourceMp3 + stimTones[0][0],
    data: {
      correct_response: correct_responses[0][1],
      //condition: correct_responses[0][0],
...
var test_trials5 = jsPsych.randomization.repeat(test_stimuli_block5, 12);
/*  
...
var test_block5 = {
  type: 'single-audio',
  timeline: test_trials5,
  data: {
    trial_id: "stim",
    exp_stage: "test_block_5"
  },
  is_html: true,
  choices: [32],
  timing_stim: 1000,
  ...
  text: '<div class = bigBox>'+
  			'<div class = welcomeBox>'+
  				'<p class = center-block-text><font color = "white">'+
  					'Thank you for participating! You have completed the experiment.'+
  				'</font></p>'+
  			'</div>'+
  		'</div>',
  cont_key: [13],
...
gng_food_experiment.push(attention_node)
gng_food_experiment.push(reset_block)
gng_food_experiment.push(test_block5);
gng_food_experiment.push(attention_node)
gng_food_experiment.push(reset_block)
